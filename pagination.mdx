...
QuickBooks frequently needs to paginate responses to avoid timeouts and other issues. Usually this would require tracking iterator metadata and making multiple requests to QuickBooks. QuBe Sync handles this for you automatically.

## Auto-pagination to the rescue

QuBe Sync can automatically paginate any request to QuickBooks that uses the `iterator` attribute. Each result page will be sent to the `webhook_url` provided with the original request. Each webhook will contain the `page` and `page_request_id` attributes to help you distinguish between pages.
